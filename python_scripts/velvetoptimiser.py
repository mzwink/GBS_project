output = open("velvetoptimiser.sh", 'w')

output.write("!/bin/bash\n#PBS -N VelvetOptimiser\n#PBS -q batch\n#PBS -l nodes=1:ppn=4:HIGHMEM\n#PBS -l walltime=480:00:00\n\n")
output.write("cd /lustre1/mz00685/mice_alignment/combined_runs/paired_unmapped_reads/\n")
...
for b in barcodes:
    ...
    read1=str(b)+"_unmapped_1.fq"
    read2=str(b)+"_unmapped_2.fq"

    output.write("perl /usr/local/apps/velvetoptimiser/2.2.5/VelvetOptimiser.pl -f 'shortPaired -fastq " + str(read1) +" -shortPaired2 -fastq " + str(read2) + "' -t 3\n")
