...
def read_depth_restr_cuts(read_cov, snp_dict):
    ...
    output_name = read_cov.replace("_cov", "_snps")
...
for i in range(0,96):
    ...
    for chr_num in range(start,20):
        ...
        snp_dict = make_snp_dict("chr_snps/chr" + str(chr_num) + "_snps.vcf")
        read_depth_restr_cuts(str(sample) + "_read_cov/" + str(sample) + "_chr" + str(chr_num) + "_cov_stats.txt",  snp_dict)
    #read_depth_restr_cuts("chr_restr_sites/chr_1_restr_enzyme_sites.txt", coverage_dict)
    #print(restr_enzyme_dict["chr10"])

    snp_dict = make_snp_dict("chr_snps/chrM_snps.vcf")
    read_depth_restr_cuts(str(sample) + "_read_cov/" + str(sample) + "_chrM_cov_stats.txt",  snp_dict)

    snp_dict = make_snp_dict("chr_snps/chrX_snps.vcf")
    read_depth_restr_cuts(str(sample) + "_read_cov/" + str(sample) + "_chrX_cov_stats.txt",  snp_dict)

    snp_dict = make_snp_dict("chr_snps/chrY_snps.vcf")
    read_depth_restr_cuts(str(sample) + "_read_cov/" + str(sample) + "_chrY_cov_stats.txt",  snp_dict)
